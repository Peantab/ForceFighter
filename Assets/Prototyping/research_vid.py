...
while True:
    ...
    gradient = np.zeros((100, 50))
    for (i, rect) in enumerate(detect):
        shape = predictor(gray, rect)
        lx, ly, lw, lh = get_eyes(shape, 0)
        rx, ry, rw, rh = get_eyes(shape, 6)
        shape = face_utils.shape_to_np(shape)
        (x, y, w, h) = face_utils.rect_to_bb(rect)
        offset = 3
        eye_image_left = gray[ly-5:ly + lh + 5, lx - 5: lx + lw + 5]
        eye_image_right = gray[ry-5:ry + rw + 5, rx: rx - 5 + rw + 5]
        gray_blurred_r = cv2.blur(eye_image_left, (25, 25))
        gray_blurred_l = cv2.blur(eye_image_right, (25, 25))
        # ql = np.gradient(gray_blurred_l)
        # vl = (ql[0] + ql[1]) / 2
        (minVal, maxVal, minLoc, maxLoc) = cv2.minMaxLoc(gray_blurred_l)

        cv2.circle(gray, (lx + minLoc[0], ly + minLoc[1]), 3, (255, 255, 255), 1)

        # qr= np.gradient(gray_blurred_r)
        # vr = (qr[0] + qr[1]) / 2
        (minVal, maxVal, minLoc_R, maxLoc) = cv2.minMaxLoc(gray_blurred_r)

        cv2.circle(gray, (rx + minLoc_R[0], ry + minLoc_R[1]), 3, (255, 255, 255), 1)
